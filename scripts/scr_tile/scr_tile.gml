/**
 * Represents a tile property with a name, value, and associated buffs.
 *
 * @param {string} _name - The name of the tile property.
 * @param {number} _value - The numeric value assigned to the property.
 * @param {bool} _playable - The state whether the tile is playable or not
 * @param {Array} _buffs - A collection of buffs or modifiers linked to the property.
 *
 */
function TileProperty(_name, _value, _playable = true, _buffs=[]) constructor {
   name = _name;
   value = _value;
   buffs = _buffs;
   playable = _playable;
   placed = false;
   
   static setPlaced = function () {
      placed = true;
   }
}


/**
 * This function will draw a single tile
 * 
 * @parameter {Real} _x The x coordinate of the tile
 * @parameter {Real} _y The y coordinate of the tile
 * @parameter {String} _type_id The id of the type of the tile
 * @parameter {Constant.Color} _color The drawn color of the tile
 */

function draw_number_tile(_x, _y, _type_id, _color=c_white){
   var _store_halign = draw_get_halign();
   var _store_font = draw_get_font();
   draw_set_halign(fa_center);
   
   var _tiles_properties = parse_tiles_properties_json()
   var _current_tile = struct_get(_tiles_properties, _type_id);
   
   // Draw the tile's label
   draw_sprite_ext(spr_tile, _current_tile.sprite_img_id, _x, _y, 1, 1, 0, _color, 1);
   
   // Draw the score of the tile
   draw_set_font(global.fnt_text);
   draw_text(
      _x + sprite_get_width(spr_tile) - 20,
      _y + sprite_get_height(spr_tile) - string_height("0") - 8,
      string(_current_tile.score)
   );
   
   draw_set_font(_store_font);
   draw_set_halign(_store_halign);
}

